...
from tuesmon_ncurses.core import TuesmonCore, StateMachine
from tuesmon_ncurses import controllers
...
def test_if_client_is_not_authenticated_the_login_view_is_shown_on_startup():
    ...
    core = TuesmonCore(executor, settings, authenticated=False)
...
def test_if_client_is_authenticated_the_projects_view_is_shown_on_startup():
    executor = factories.patched_executor()
    core = TuesmonCore(executor, settings, authenticated=True, draw=False)
